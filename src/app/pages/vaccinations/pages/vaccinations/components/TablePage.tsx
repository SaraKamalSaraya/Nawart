...
export default function VaccinationsTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (res?.status == '404') {
        return navigate('/no-items?to=vaccinations')
      }
      ...
      SwalConfirmAlert(
        ...
        async () => {
          for (const id of itemsToDelete) {
            await deleteMethod(`/vaccinations/${id}`)
          }
  ...
  const headers = ['#', 'Title', 'Description', 'Features', 'Medical Services', 'Created At', 'Updated At']
