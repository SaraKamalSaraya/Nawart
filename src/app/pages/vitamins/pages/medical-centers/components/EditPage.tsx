...
export default function EditMedicalCenter() {
  ...
  const [vitamisList, setvitamisList] = useState<any[]>([])
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const res = await GetMethod(`/medical-center-vitamins/${id}`)
      setFormData(res?.data?.data)
    }

    const fetchvitamisList = async () => {
      const res = await GetMethod('/vitamins')
      setvitamisList(res?.data?.vitamins)
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const response = await postMethod(`/medical-center-vitamins/update/${id}`, formDataToSend)
    setLoading(false)
  ...
  return (
    <Container className='mt-5 card p-8'>
      <h2>{t('Edit Medical Center For Vitamins')}</h2>
      <Form onSubmit={handleSubmit} className='mt-5 '>
        <div className='row w-100'>
          ...
          <Form.Group className='my-5' style={{ width: '45%' }} controlId='vitamis_id'>
            <Form.Label>{t('Vitamin')}</Form.Label>
            <Form.Select onChange={handleChange} value={formData?.vitamis_id}>
              <option value={0}>{t('Select')}</option>
              {vitamisList?.map((vitamins: any) => (
