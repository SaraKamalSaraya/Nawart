...
export default function EditPatientPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const user = res?.data.data.find((item: any) => item.id == id)
      setFormData({ ...user ,image:null})
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const response = await postMethod(`/user/update/${id}`, formDataToSend)
    setLoading(false)
