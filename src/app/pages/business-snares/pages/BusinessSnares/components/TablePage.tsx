...
export default function BusinessSnaresTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (res?.status == '404') {
        return navigate('/no-items?to=business-snares')
      }
      setData(res?.data?.businessSnares)
    }
      ...
      SwalConfirmAlert(
        ...
        async () => {
          for (const id of itemsToDelete) {
            await deleteMethod(`/business-snares/${id}`)
          }
  ...
  const headers = ['#','Title', 'Description','Features' , 'Created At', 'Updated At']
