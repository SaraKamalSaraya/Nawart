...
export default function EditHealthcarePackagePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const response = await postMethod(`/healthcare_packages/update/${id}`, formDataToSend)
    setLoading(false)
  ...
  return (
    <Container className='mt-5 card p-8'>
      <h2>{t('Edit Healthcare Package')}</h2>
      <Form onSubmit={handleSubmit} className='mt-5 '>
        <div className='row w-100'>
          ...
          <Form.Group className='my-5' style={{ width: '45%' }} controlId='instructions'>
            ...
            <Form.Control as='textarea' rows={1} type='text' placeholder={t('Enter Instructions')} value={formData.instructions} onChange={handleChange}
            />
          </Form.Group>
