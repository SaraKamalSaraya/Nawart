...
export default function EditBodyFunctionPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const res = await GetMethod(`/body_functions/${id}`)
      const data = res?.data?.bodyFunction
      const analysis_ids = data?.analyses?.map((analysis: any) => analysis.id)
      const analysis_group_ids = data?.analysis_group?.map((analysis_group: any) => analysis_group.id)
      setFormData({...data,icon:null, analysis_ids, analysis_group_ids})
      setIconPreview(data?.icon)
      console.log(res)
      console.log(data,analysis_ids,analysis_group_ids)
    }
    fetchAnalysisList()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const response = await postMethod(`/body_functions/update/${id}`, formDataToSend)
    setLoading(false)
