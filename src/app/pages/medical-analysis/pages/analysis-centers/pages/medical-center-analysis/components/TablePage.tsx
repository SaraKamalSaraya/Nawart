...
export default function MedicalCenterAnalysisTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = await getMethod('/medical-center-analysis')
      if (res?.status == '404') {
        return navigate('/no-items?to=medical-analysis/analysis-centers/analysis')
      }
      ...
      SwalConfirmAlert(
        ...
        async () => {
          for (const id of itemsToDelete) {
            await deleteMethod(`/medical-center-analysis/${id}`)
          }
  ...
  const headers = ['#', 'Medical Center', 'Analysis', 'Created At', 'Updated At']
      ...
      >
        {data?.map((item, index) => (
          ...
          >
            ...
            <td><Link to={`/medical-analysis/individual-analyses/classification/individual-analyses/edit/${item.analysis.id}`}>{item.analysis.title}</Link></td>            
