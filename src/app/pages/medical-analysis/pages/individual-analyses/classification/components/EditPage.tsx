...
export default function EditClassificationPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = await GetMethod(`/tasnif/${id}`)
      if (res?.status == '404') {
        return navigate('/no-items?to=medical-analysis/individual-analyses/classification')
      }
      const tasnif = res?.data?.data
      setFormData({ ...tasnif , icon: null})
      setIconPreview(tasnif.icon)
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const response = await postMethod(`/tasnif/update/${id}`, formDataToSend)
    setLoading(false)
